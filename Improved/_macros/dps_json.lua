...
function each_file(rgd)
	...
	if show and rgd.GameData and rgd.GameData.weapon_bag then
		...
		--print(string.format("# %s. [%d, %d] Clip: %d, Damage scale:", rgd.name, weapon.range.min, weapon.range.max, clip))
		print(string.format('[%q, [', rgd.name))
		...
		function output(distance)
			print(string.format("[%d, %.4f],", weapon.range.mid[distance], getDPS(distance)))
		end
		output("short")
		output("medium")
		output("long")
